  ...
  try {
    ...
    pythonProcess.stdout.on("data", (data) => {
      ...
      if (
        ...
        console.log(
          "Detected Python API startup message. API may be ready soon."
        );
      }
    });
    ...
    pythonProcess.stderr.on("data", (data) => {
      ...
      if (error.includes("Address already in use")) {
        console.error(
          "Python API port 8000 is already in use. Possibly another instance is running."
        );
    ...
    const healthCheckInterval = setInterval(async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            `API responded with status: ${response.status}, but may not be fully ready yet`
          );
        }
      } catch (error) {
        if (error.code === "ECONNREFUSED") {
          console.log(
            "API server not accepting connections yet, will retry..."
          );
        } else {
      ...
      if (healthCheckAttempt >= maxHealthCheckAttempts) {
        clearInterval(healthCheckInterval);
        console.error(
          `Failed to connect to Python API after ${maxHealthCheckAttempts} attempts.`
        );

        if (
          objectDetection.pythonProcess &&
          objectDetection.pythonProcess.exitCode === null
        ) {
          console.log(
            "Python process is still running but API is not responsive. You may need to check for errors."
          );
        }
      }
...
const server = createServer(async (req, res) => {
  ...
  if (req.url === "/api/session/start" && req.method === "POST") {
    ...
    req.on("end", async () => {
      try {
        ...
        console.log(
          `[SESSION START] New session started: ${activeSessionId}, duration: ${duration}min, count: ${count}`
        );
        ...
        console.log(
          `[SESSION START] Notified ${clients.browsers.size} browsers about new session`
        );
        ...
        setTimeout(async () => {
          if (activeSessionId === sessionId.toString()) {
            try {
              console.log(
                `[SESSION AUTO-END] Auto-ending session ${sessionId} after ${duration} minutes`
              );
              ...
              console.log(
                `Session ${sessionId} auto-ended after ${duration} minutes`
              );
            } catch (error) {
  ...
  if (
    req.url.match(/^\/api\/session\/[^\/]+\/detections/) &&
    req.method === "GET"
  ) {
...
function sendCameraInfo(cameraId) {
  ...
  // Otherwise send all camera info
    ...
    const allCameras = Array.from(cameraMetadata.entries()).map(
      ([id, info]) => ({
        ...info,
...
/**
 * Process video frames - optimized for 5fps AI processing
...
function processVideoFrame(frame, cameraId) {
  ...
  // Store only the latest frame from this camera
  streamSettings.latestFrames.set(cameraId, {
    data: frame,
    timestamp: now,
    size: frame.length
  });

  // Cleanup old frames (keep only last 5 seconds worth)
  const cleanupThreshold = now - 5000;
  for (const [id, frameInfo] of streamSettings.latestFrames.entries()) {
    if (frameInfo.timestamp < cleanupThreshold && id !== cameraId) {
      streamSettings.latestFrames.delete(id);
    }
  }
  ...
  // Log frame statistics every 10 seconds
  ...
  if (now - lastLog > 10000) {
    ...
    console.log(
      `[FRAME STATS] Camera ${cameraId}: ${fps.toFixed(
        1
      )} fps, ${frameSize}KB per frame`
    );
    streamSettings.lastLogTime.set(cameraId, now);
    streamSettings.frameCounter.set(cameraId, 0);
  }

  // AI detection at exactly 5fps (200ms intervals)
  if (objectDetection.enabled && clients.ai && activeSessionId) {
    ...
    if (now - lastDetection >= objectDetection.detectionInterval) {
      ...
      console.log(`[DETECTION] Triggering AI analysis for camera ${cameraId}`);
...
/**
 * Send current frame to AI service - optimized for 5fps
 * @param {Buffer} frame - Binary JPEG frame data
 * @param {string} cameraId - ID of the camera source
 */
function sendFrameToAI(frame, cameraId) {
  if (
    !activeSessionId ||
    !clients.ai ||
    clients.ai.readyState !== WebSocket.OPEN
  ) {
    return;
  }

  if (!cameraId || !frame || frame.length === 0) {
    console.warn("[AI FRAME] Invalid frame or camera ID");
    return;
  }

  // Simple concurrency check - only allow 1 concurrent detection per camera
  const processingKey = `${cameraId}_processing`;
  if (objectDetection.detectionLog.get(processingKey)) {
    return; // Skip if already processing for this camera
  }

  try {
    // Mark as processing
    objectDetection.detectionLog.set(processingKey, Date.now());

    // Send optimized single message with embedded metadata
    const frameData = {
      ...
      image: frame.toString("base64") // Convert buffer to base64
    };

    console.log(
      `[AI FRAME] Sending frame from camera ${cameraId} (${Math.round(
        frame.length / 1024
      )}KB)`
    );

    clients.ai.send(JSON.stringify(frameData), (err) => {
      if (err) {
        console.error(`[AI FRAME] Send error: ${err.message}`);
        objectDetection.detectionLog.delete(processingKey);
      }
    });
  } catch (error) {
...
wss.on("connection", (ws, req) => {
  ...
  if (isCamera) {
    ...
    console.log(
      `[CAMERA CONNECTION] Camera client connected from ${clientIp}, waiting for identification...`
    );
      ...
      try {
        ...
        if (isJpegData(data)) {
          ...
          if (cameraId) {
            ...
            console.warn(
              "[CAMERA FRAME] Received JPEG frame but camera ID is not yet known"
            );
          }
          return;
        }
        ...
        if (typeof data === "string" || data.toString !== undefined) {
          try {
            ...
            if (message.id) {
              ...
              if (!clients.cameras.has(cameraId)) {
                ...
                console.log(
                  `[CAMERA REGISTRATION] Camera identified as: ${cameraId}`
                );
              }
              ...
              if (message.type === "camera_info") {
                ...
                console.log(
                  `[CAMERA METADATA] Updated metadata for camera: ${cameraId}`,
                  {
              ...
              // Store frame metadata for next binary frame
                ...
                if (Math.random() < 0.01) {
                  console.log(
                    `[CAMERA METADATA] Received frame metadata for ${cameraId}`
                  );
                }
              }
            }
            ...
            if (data instanceof Buffer && isJpegData(data)) {
              if (cameraId) {
                console.log(
                  `Processing detected JPEG frame: ${data.length} bytes`
                );
                processVideoFrame(data, cameraId);
              }
            } else {
              console.warn(
                `Received non-JSON text from camera: ${data
                  .toString()
                  .substring(0, 50)}...`
              );
            }
          ...
          console.log(
            `[CAMERA FRAME] Received binary frame from ${cameraId}: ${data.length} bytes`
          );
          processVideoFrame(data, cameraId);
        } else {
          console.warn(
            `[CAMERA FRAME] Received binary data but no camera ID set`
          );
        }
      } catch (error) {
    ...
    // Update the AI WebSocket message handler
      ...
      try {
        ...
        switch (data.type) {
          case "detection_response":
            ...
            break;
          case "pong":
            console.log("[AI PONG] Received pong from AI service");
            break;
          default:
            console.log(`[AI MESSAGE] Unknown type: ${data.type}`);
        }
      } catch (error) {
        console.error(`[AI MESSAGE] Parse error: ${error.message}`);
      }
    });
  } else {
    // Browser client
    ...
    if (activeSessionId) {
      console.log(
        `[BROWSER SESSION] Sending active session status to new browser: ${activeSessionId}`
      );
      ws.send(
        JSON.stringify({
          type: "session_status",
    ...
    ws.on("message", async (message) => {
      try {
        ...
        if (data.type === "get_camera_list") {
          ...
          console.log(
            `Browser client subscribed to session: ${data.sessionId}`
          );
        }
  ...
  ws.on("close", (code, reason) => {
    ...
    if (disconnectedCameraId) {
      ...
      console.log(
        `Camera ${disconnectedCameraId} disconnected - Code: ${code}`
      );
      ...
      for (const browser of clients.browsers) {
        if (browser.readyState === WebSocket.OPEN) {
          try {
            ...
            console.error(
              `Error notifying browser of camera disconnect: ${error.message}`
            );
          }
        }
      }
...
function checkAIServiceHealth() {
  ...
  if (objectDetection.processingCount > 0) {
    ...
    const lastDetectionTime = Math.max(
      ...Array.from(objectDetection.lastDetectionTime.values(), 0)
    );

    // If no detections for over 30 seconds but processingCount > 0, reset it
    if (now - lastDetectionTime > 30000) {
      console.log(
        `Resetting stuck processing count from ${objectDetection.processingCount} to 0`
      );
...
/**
 * Process AI detection response - optimized for performance
 * @param {Object} data - AI response data
 */
async function processAIResponse(data) {
  try {
    const { camera_id, results, timestamp, session_id } = data;

    if (!camera_id || !results) {
      console.warn("[AI RESPONSE] Invalid response data");
      return;
    }

    // Clear processing flag
    const processingKey = `${camera_id}_processing`;
    objectDetection.detectionLog.delete(processingKey);

    // Store latest detection results (only keep most recent)
    objectDetection.detectionResults.set(camera_id, {
    ...
    // Add to database if session is active and matches
    if (session_id && session_id === activeSessionId) {
      try {
        await addDetectionToSession(activeSessionId, {
          cameraId: camera_id,
          detections: results.detections || [],
          carCount:
            results.detections?.filter((d) => d.class_name === "car").length ||
            0,
          accidentCount:
            results.detections?.filter((d) => d.class_name === "accident")
              .length || 0,
          inference_time: results.inference_time,
        ...
        console.error(`[AI RESPONSE] Database error: ${dbError.message}`);
      }
    }

    // Broadcast to browsers (optimized payload)
    broadcastDetectionResults(camera_id, {
      detections: results.detections || [],
      inference_time: results.inference_time,
      timestamp: timestamp,
      image_size: results.image_size
    ...
    if (results.detections && results.detections.length > 0) {
      const analysis = analyzeTrafficDensity(results.detections);
      handleTrafficRedirection(camera_id, analysis);
    }
  } catch (error) {
    console.error(`[AI RESPONSE] Processing error: ${error.message}`);
  }
}

/**
 * Analyze traffic density from detections
 * @param {Array} detections - Detection results
 * @returns {Object} Traffic analysis
 */
function analyzeTrafficDensity(detections) {
  const carCount = detections.filter((d) => d.class_name === "car").length;
  const accidentCount = detections.filter(
    (d) => d.class_name === "accident"
  ).length;

  let density = "low";
  if (carCount > 10 || accidentCount > 0) density = "high";
  else if (carCount > 5) density = "moderate";

  return {
    density,
    vehicle_count: carCount,
...
server.listen(PORT, async () => {
  ...
  console.log(
    `[SYSTEM STATUS] Object detection enabled: ${objectDetection.enabled}`
  );
  console.log(
    `[SYSTEM STATUS] Confidence threshold: ${objectDetection.confidenceThreshold}`
  );
  console.log(
    `[SYSTEM STATUS] Detection interval: ${objectDetection.detectionInterval}ms`
  );
  console.log(
    `[SYSTEM STATUS] Max concurrent: ${objectDetection.maxConcurrent}`
  );
  console.log(
    `[SYSTEM STATUS] Rate limit: ${objectDetection.rateLimit.maxRequestsPerMinute} req/min`
  );
  ...
  setTimeout(async () => {
    ...
    if (apiHealthy) {
      console.log(
        "[STARTUP] Object detection API is ready. Detection enabled."
      );
      objectDetection.enabled = true;
      objectDetection.errorCount = 0;
      objectDetection.processingCount = 0;
      ...
      console.log(
        "[STARTUP] Object detection API is not available. Detection disabled."
      );
      ...
      setTimeout(async () => {
        ...
        if (retryHealthy) {
          console.log(
            "[STARTUP] Object detection API is now ready on second attempt. Detection enabled."
          );
          objectDetection.enabled = true;
          objectDetection.errorCount = 0;
          objectDetection.processingCount = 0;
          ...
          console.log(
            "[STARTUP] Python API still not ready after retry. Will continue monitoring..."
          );
        }
      }, 5000);
    }
    ...
    // Setup periodic frame broadcasts
    setInterval(() => {
      broadcastFramesToBrowsers();
    }, 200); // Send frames at 5fps (200ms)
  }, 10000);
});

/**
 * Broadcast latest frames from cameras to browser clients
 */
function broadcastFramesToBrowsers() {
  // Skip if no browsers connected
  if (clients.browsers.size === 0) return;
  
  // For each camera that has frames
  for (const [cameraId, frameInfo] of streamSettings.latestFrames.entries()) {
    if (!frameInfo || !frameInfo.data) continue;
      ...
      if (browser.readyState === WebSocket.OPEN) {
        try {
          browser.send(JSON.stringify({
            ...
            frame: frameInfo.data.toString('base64')
          }));
        } catch (error) {
          console.error(`Error broadcasting frame: ${error.message}`);
        }
      }
    }
  }
...
setInterval(() => {
  const stats = {
    activeFrames: streamSettings.latestFrames.size,
    activeCameras: clients.cameras.size,
    pendingDetections: objectDetection.detectionLog.size,
    memoryUsage: process.memoryUsage(),
    aiConnected: clients.ai ? 'YES' : 'NO'
  };
  
  console.log(`[PERFORMANCE] ${JSON.stringify(stats)}`);
  
  // Cleanup stuck detections (older than 30 seconds)
  const now = Date.now();
  for (const [key, timestamp] of objectDetection.detectionLog.entries()) {
    if (now - timestamp > 30000) {
      objectDetection.detectionLog.delete(key);
      console.warn(`[CLEANUP] Removed stuck detection: ${key}`);
    }
  }
}, 30000);
